...
1. **모델 구조를 바꾸면 양쪽을 다 손봐야 한다.**
   ...
   ②를 잊으면 무엇이 잡아내는가 — 두 가지 검사가 서로 다른 실수를 잡는다.
   - `내보내기.py` 자체는 `state_dict` 의 텐서 이름·순서·모양이 예상과 다르면 그
     자리에서 멈춘다. 다만 이 검사는 **내보내기를 실제로 실행했을 때만** 작동하고
     구조 변경만 잡으므로, 구조는 그대로 두고 에폭만 늘려 재학습한 경우(이름·모양이
     안 바뀜)는 걸리지 않는다.
   - 그 경우는 `web_version/점검.mjs` 의 0단계(지문 대조)가 잡는다. `가중치_구조.json`
     에 내보내기 당시 `mnist_cnn.pt` 의 SHA-256 을 적어 두고 매 검증마다 실제 파일과
     대조하므로, 재학습 후 재내보내기를 잊으면 `node 점검.mjs` 가 실패한다
     (`web_version/CLAUDE.md` 참고).
